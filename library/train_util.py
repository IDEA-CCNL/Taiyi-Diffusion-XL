...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def get_input_ids(self, caption, tokenizer=None, taiyi=False):
...
class FineTuningDataset(BaseDataset):
    ...
    def image_key_to_npz_file(self, subset: FineTuningSubset, image_key):
        ...
        npz_file_norm = os.path.join(subset.latents_dir, image_key + ".npz")
...
def add_sd_models_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--taiyi", action="store_true", help="load Taiyi XL Diffusion model")
...
def add_dataset_arguments(
    ...
    parser.add_argument("--latents_data_dir", type=str, default=None, help="directory for train images' latents / 学習画像データのディレクトリ")
    parser.add_argument(
    ...
    if args.log_with is None:
        ...
        if log_with in ["wandb", "all"]:
            ...
            if logging_dir is not None:
                ...
                # wandb.init(project=log_prefix)
                os.environ["WANDB_PROJECT"] = log_prefix
...
def get_hidden_states_taiyixl(
    ...
    input_ids1 = input_ids1.reshape((-1, input_ids1.size()[-1]))  # batch_size, 512
    input_ids2 = input_ids2.reshape((-1, input_ids2.size()[-1]))  # batch_size, 512
